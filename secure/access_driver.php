...
class access {
    ...
    public function registerDriver($email, $password, $salt, $mobile, $fullname) {

        // sql command
        $sql = "INSERT INTO drivers SET demail=?, dmobile=?, password=?, salt=?, dfullname=?";
    ...
    public function updateAccountDriver($email, $mobile, $fullname, $id) {

      $sql = "UPDATE drivers SET demail=?, dmobile=?, dfullname=? WHERE did=?";
      $statement = $this->conn->prepare($sql);
    ...
    public function getMyTrips($did) {
        ...
        // sql statement
        $sql = "SELECT id, email, rid, rname, rmobile, pickaddress, picklatitude, picklongitude, dropaddress,notes ,totalcharge, status, created_at FROM requests WHERE did='".$did."'" ;
